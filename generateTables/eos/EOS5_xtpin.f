			double precision ta,tb,pa,pb
			double precision t6a,t6b,p12a,p12b
			double precision logt6a,logt6b,logp12a,logp12b
			double precision dlogtsix,dlogptwelve
			double precision lts,lpt,xt,xp
			double precision nV,minusOne
			double precision dE,g1,g2
			integer fail,tEvals,pEvals,ii,jj
      ...
      irad=1    ! adds radiation corrections
			...
			ta=1.8e3
			tb=3e7
			pa=2e-2 ! these are in dyne/cm**2
c			pa=1e-3
			pb=1e18 !			
...
c -------
			...
			dlogtsix=(logt6b-logt6a)/(tEvals-1.0)
			dlogptwelve=(logp12b-logp12a)/(pEvals-1.0)

			do 10 ii=0,pEvals-1
			do 20 jj=0,tEvals-1

				lts=logt6a+dlogtsix*jj
				lpt=logp12a+dlogptwelve*ii

				tsix=10.0**lts
				ptwelve=10.0**lpt
				
				fail=0
		    rho=rhoofp (x,ztab,tsix,ptwelve,irad,fail)  ! calculate density for fixed P.
				if (fail .eq. 0) call esac(x,ztab,tsix,rho,iorder,irad,fail) ! calc EOS; use rho from rhoofp

				if (fail .ne. 0) then					
					write (*,"(8e18.9)") lts,lpt,nV,nV,nV,nV,nV,nV
				else
					dE=eos(5)
					g1=eos(8)
					g2=eos(9)/(eos(9)-1)
					xr=eos(6)
					xt=eos(7)
					write (*,"(8e18.9)") lts,lpt,log10(rho),dE,g1,g2,xr,xt
				endif    
...
c*************************************************************************
      subroutine esac (xh,ztab,t6,r,iorder,irad,fail)
...
c__________
  ...
  123 continue
     ...
     x    ,epl(mh,it,ir),xx(mf),xx(mg),xx(mh))
          ...
          write(*,'(" problem it ir,l3,k3,iq,ip=", 6i5)') it,ir TMP DBG
     x    ,l3,k3,iq,ip
...
c
  ...
  124 continue
   ...
   61 write(*,'(" Mass fractions exceed unity (61)")')
      stop
c   62 write(*,'(" T6/LogR outside of table range (62)")')
   62 fail=1
			return      
c   65 write(*,'("T6/log rho in empty region od table (65)")')
c      write (*,'("xh,t6,r=", 3e12.4)') xh,t6,r
   65 fail=2
			return
...
c********************************************************************
      function rhoofp(x,ztab,t6,p,irad,fail)
      parameter (mx=5,mv=10,nr=169,nt=197)
      common/lreadco/itime
      common/aeos/  xz(mx,mv,nt,nr),  
     . t6list(nr,nt),rho(nr),t6a(nt),esk(nt,nr),esk2(nt,nr),dfsx(mx)
...
c--------------------------------------------------------------------
     ...
     x   6* 98, 4* 97, 95, 94, 6* 87/
      ...
      if (itime .ne. 12345678) then
      call esac (0.7,.02,1.,0.001,1,0,fail)
				if (fail .eq. 1) return
      endif
...
c     	write (*,'("pnr, pmax,pmin=",3e14.4)') pnr,pmax,pmin
      ...
      call esac (x,ztab,t6,rhog1,1,0,fail)
			if (fail .eq. 1) return
      ...
      p1=eos(1)
        if(p1 .gt. pnr) then
          ...
          call esac (x,ztab,t6,rhog2,1,0,fail)
					if (fail .eq. 1) return
    ...
    1 continue
      ...
      call esac (x,ztab,t6,rhog3,1,0,fail)
			if (fail .eq. 1) return
